...
$target_file = $destination_path.'uploads/'.date("YmdHis"). basename( $_FILES["fileToUpload"]["name"]);
$uploadOk = 1;
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        $target_file = preg_split("/stock/", $target_file);
        $target_file =  'http://192.168.43.15/stock/'.$target_file[1];
